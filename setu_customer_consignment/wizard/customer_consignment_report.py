...
class CustomerConsignmentReport(models.TransientModel):
    _name = 'customer.consignment.report'
    _description = "Customer Consignment Report"
    ...
    products_ids = fields.Many2many("product.product", 'customer_consignment_report_product_rel',
                                    'customer_consignment_report_id',
                                    'products_id',
                                    string="Products",
    ...
    def View_report(self):
        stock_data = self.get_consignment_customer_data()
        # print (stock_data)
        ...
        return {
            'name': _('Customer Consignment Analysis Report'),
            'domain': [('wizard_id', '=', self.id)],
            'res_model': 'customer.consignment.analysis.report',
            'view_mode': 'tree,graph,pivot,form',
            'type': 'ir.actions.act_window',
            'context': {'search_default_consignee_id_groupby': 1},
            'help': """
    ...
    def get_consignment_customer_data(self):
        """
        ...
        query = """
                Select * from get_consignment_customer_data('%s','%s','%s','%s','%s','%s')
    ...
    def create_data(self, data):
        return self.env['customer.consignment.analysis.report'].create(data)

class CustomerConsignmentAnalysisReport(models.TransientModel):
    _name = 'customer.consignment.analysis.report'
    _description = """Customer Consignment Analysis Report"""
    ...
    wizard_id = fields.Many2one("customer.consignment.report")
